...
- Installs `openkat-report`, a host CLI to list reports and fetch report JSON from the local octopoes/bytes APIs (`openkat-report list` / `get` / `meta` / `findings` / `history` / `timeline`). The `findings [--full]` subcommand resolves a concatenated/multi report to its asset sub-reports and merges their report_data (findings, severity, occurrences, summary) into a single JSON document. All commands accept `-t <valid_time>` for bitemporal time-travel, `history <reference>` returns the transaction timeline of a single OOI (e.g. a Finding), and `timeline <reference> [--from D] [--to D] [--step N]` samples a finding's presence and severity over a period.
